...
export const mangaScanlinesPreset: EffectPreset = {
  ...
  advancedControlSchema: [
    { id: "lineSpacing", name: "Line Spacing", type: "range", min: 2, max: 24, step: 1, defaultValue: 2 },
    ...
    { id: "threshold", name: "Threshold", type: "range", min: 0, max: 255, step: 1, defaultValue: 80 },
  ...
  intensityMapper: (intensity, overrides): ResolvedPresetParameters => ({
    ...
    lineSpacing: resolveOverride(overrides, "lineSpacing", 2),
    lineWidth: resolveOverride(overrides, "lineWidth", 2),
    angle: resolveOverride(overrides, "angle", 0),
    threshold: resolveOverride(overrides, "threshold", 80),
  ...
  createPipeline: (params): EffectPipeline => {
    return (source: PixelBuffer) => {
      ...
      const lineSpacing = (params.lineSpacing as number) ?? 2;
