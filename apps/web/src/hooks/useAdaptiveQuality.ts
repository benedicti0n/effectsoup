...
export function useAdaptiveQuality() {
  ...
  const scaleIndexRef = useRef(scaleIndex);
  ...
  const reportDuration = useCallback((durationMs: number) => {
    if (durationMs > BUDGET_MS) {
      ...
      if (slowFramesRef.current >= STEP_DOWN_THRESHOLD && scaleIndexRef.current < SCALES.length - 1) {
      ...
      slowFramesRef.current = 0;
      if (fastFramesRef.current >= STEP_UP_THRESHOLD && scaleIndexRef.current > 0) {
        setScaleIndex((idx) => idx - 1);
        fastFramesRef.current = 0;
      }
    }
